...
class MakeDataSet:
    ...
    def create_npy_from_mhd(self):
        ...
        For IMAGE folder        
        with open(S_META_DIR+"/"+'nodulemhd.csv') as f:
            lines = f.readlines()
            count = 0
            for line in lines:
                count += 1
                ...
                lungimgname=S_IMAGE_DIR+"/"+line+".mhd"
                ...
                for index in range(i):
                    ...
                    if not os.path.exists(self.img_path+"/"+linedir):
                        os.makedirs(self.img_path+"/"+linedir)   
                    lungimgnm=self.img_path+line;
                    print(lungimgnm)
        ...
        with open(S_META_DIR+"/"+'nodulemaskmhd.csv') as f:
            lines = f.readlines()
            count = 0
            for line in lines:
                count += 1
                ...
                lungimgname=S_MASK_DIR+"/"+line+".mhd" #TOCHANGE
                lungimg=sitk.ReadImage(lungimgname) 
                lungimg= sitk.GetArrayFromImage(lungimg)
                print(type(lungimg))
                print(lungimg.shape)
                ...
                for index in range(i):
                    ...
                    # Naming of each file: NI= Nodule Image, MA= Mask Original
                    #nodule_name = "{}/{}_NI{}_slice{}".format(pid,pid[-4:],prefix[nodule_idx],prefix[nodule_slice])
                    if not os.path.exists(self.mask_path+"/"+linedir): #TOCHANGE
                        os.makedirs(self.mask_path+"/"+linedir)     #TOCHANGE
                    lungimgnm=self.mask_path+line;  #TOCHANGE
